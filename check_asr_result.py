...
meta_path = f'{model_name}_path.txt'
with open(meta_path, 'r') as fin:
    data = fin.read().splitlines()

hypotheses = []
references = []
for i, line in tqdm(enumerate(data), desc=f'Processing {model_name}...'):
    # if i +1 != 112:
    #     continue

    gt_path, gt_text, infer_path = line.strip().split('|')
    ...
    try:
        ...
        with open(f'asr_{model_name}.txt', 'a') as f:
            f.write(f'{i + 1}|{gt_text_clean}|{trans_text_clean}\n')
